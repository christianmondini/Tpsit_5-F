...
void Pari(char *parola,char *pari){
    int j=0;
   for(int i=0;i<strlen(parola);i++){
    if(i%2!=0){//Prendo i dispari perche' la parola parte da 1 e nn da 0
            pari[j]=parola[i];
            j++;
...
void Dispari(char *parola,char *dispari){
    int j=0;
   for(int i=0;i<strlen(parola);i++){
    if(i%2==0){//Prendo i pari perche' la parola parte da 1 e non da 0
            dispari[j]=parola[i];
            j++;
...
int main(int argc, char *argv[])
{
    ...
    int lunga1;
    int lunga2;
    if((lunghezza%2)==0){//Se parola lunghezza pari: le lettere pari e dispari saranno uguali
        lunga1=(lunghezza/2);
        lunga2=(lunghezza/2);
    }else{//invece se lunghezza parola dispari
        lunghezza+=1;
   ...
   /* printf("LUNGA1 %d",lunga1);
    ...
    char stringap[lunga1];
    char stringad[lunga2];
    do
    {
        ...
        {
        case 1:
            int vocali = Vocali(parola);
            printf("Le vocali sono %d\n", vocali);
            break;
        case 2:
            int consonanti = Consonanti(parola);
